...
// NOTE FROM DEVIN: not using this function right now until re-work can be done to make it Qt safe
void readFiletoMem(State8080* state, char* fileName, uint32_t offset) {
    QTextStream out(stdout);
    FILE* fp = fopen(fileName, "rb");
    if (fp == NULL) {
        out << "ERROR: could not open file %s\n";
        exit(1);
    }
...
void readInvaderstoMem(State8080* state) {
    readFiletoMem(state, "../Qt_GUI/roms/invaders.h", 0x0000);
    readFiletoMem(state, "../Qt_GUI/roms/invaders.g", 0x0800);
    readFiletoMem(state, "../Qt_GUI/roms/invaders.f", 0x1000);
    readFiletoMem(state, "../Qt_GUI/roms/invaders.e", 0x1800);
}

/* Initializes cpu state struct */
State8080* init8080CPU() {
    State8080* state = (State8080*)calloc(1, sizeof(State8080));
    state->memory = (uint8_t*)calloc(1, 0x10000); //16000 bytes allocated for memory
    return state;
}
